import { MusicHandler } from './audio';
import { Instrument } from './instrument';
import { Sheet, shiftOctave, str2Sheet } from './music';
...
export const mainTheme: MusicHandler = new MusicHandler(
  [mainThemeInstrument0, mainThemeInstrument1],
  [
    shiftOctave(mainTheme00Sheet, +0), // 0
    shiftOctave(mainTheme00Sheet, -1), // 1
    shiftOctave(mainTheme10Sheet, +0), // 2
    shiftOctave(mainTheme11Sheet, +1), // 3
    shiftOctave(mainTheme10Sheet, +1), // 4
  ],
  {
    0: [0, 1, 0, 1],
    1: [2, 3, 3, 4],
...
export function playMusic(): void {
  ...
  Object.keys(mainTheme.assignments).forEach((insID: string) => {
    mainTheme.assignments[+insID].forEach((shtID: number, i: number) => {
      ...
      mainTheme.instruments[+insID].playSheet(mainTheme.sheets[+shtID], dt);
    });
  });
